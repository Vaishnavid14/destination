...
<form action="register_room.php" method="POST">
  <div class="container">
    ...
    <select name="type">
      ...
      <option value="4">Jr. Suite</option>
      <option value="5">Executive Suite</option>
    </select>
	 <input type="text" placeholder="Room Price" name="price" required>
   <label>Sleeps</label>
    <select name="sleep">
      <option value="">Select</option>
    ...
    <select name="hotel">
      <?php
      $get_q = "select * from hotel";
                    $run_q = mysqli_query($mysqli,$get_q);
                    $row_cats=mysqli_fetch_array($run);
                    while($row_h=mysqli_fetch_array($run_q))
                            {
                              
                                $cat_id=$row_h['hotel_id'];
                                $cat_title=$row_h['hotel_name'];    
