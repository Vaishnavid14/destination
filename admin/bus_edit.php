...
<form action="bus_update.php" method="POST">
  <div class="container">
  <?php
	$result = mysqli_query($mysqli,"SELECT * FROM bus_list WHERE bus_id ='$id'");
	while($row = mysqli_fetch_array($result))
	{
	echo"<input type='hidden' name='id' value='{$row['bus_id']}' required>";
    echo"<input type='text' placeholder='BUs Number' name='num' value='{$row['bus_no']}' required>";
    echo"<input type='text' placeholder='Total Seats' name='seat' value='{$row['total_seats']}' required>";
    echo"<label><b>Type</b>";
    echo"<select name='type'>
      ...
      <option value='4'>Non Ac</option>
    </select>";
    echo"</label>";
    
    echo"<input type='text' placeholder='Ticket per km' name='ticket' value='{$row['ticket_per_km']}' required>";
